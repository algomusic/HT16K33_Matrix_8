/*
 * Font3x3Quadrants.ino
 *
 * Displays four characters on an 8x8 LED matrix using the 3x3 font:
 *
 *   +------+------+
 *   | char | char |   top: two lowercase letters (3x3 with spacing)
 *   +------+------+
 *   | dig  | dig  |   bottom: two digits (3x4, left-justified)
 *   +------+------+
 *
 * The 3x3 letter glyphs leave column 3 and row 3 blank within each
 * quadrant, giving clear visual separation between all four characters.
 * Digits use the full 4 rows but only 3 columns wide.
 *
...
void showQuadrants(char topLeft, char topRight, char botLeft, char botRight) {
    matrix.clear();
    font3x3_drawChar(matrix, topLeft,  0, 0);
    font3x3_drawChar(matrix, topRight, 0, 4);
    font3x3_drawChar(matrix, botLeft,  4, 0);
    font3x3_drawChar(matrix, botRight, 4, 4);
    matrix.write();
}
...
void setup() {
    ...
    Serial.println("3x3 font four-quadrant demo ready.");
